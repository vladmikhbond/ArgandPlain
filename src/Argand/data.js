...
`,
a:["φ = π", "cos = (e^(φ * i) + e^(-φ * i) ) / 2", "sin = (e^(φ * i) - e^(-φ * i) ) / 2", "π = 3.14159; e=2.71828"], m: 2,
},
{
h: "Возведение в степень и извлечение корней", 
t: `
Здесь очень пригодится показательная форма комплексных
чисел. Вспомним, что умножение - это перемножение модулей 
и сложение аргументов, т.е. углов. 
Возведение в целую степень - это многократное умножение.
Если z = re<sup>iφ</sup>, то z<sup>n</sup> = r<sup>n</sup> e<sup>inφ</sup>
    Это справедливо для любых степеней - целых, дробных,
положительных, отрицательных и даже комплексных.

Чтобы извлечь корень n-й степени, нужно возвести число в
степень 1/n. Но таким образом можно получить только один 
корень из n.
`,
a:["n = 2", "z = 1.1 * e^(i*π/4)", "z^n = z^n", "π = 3.14159; e=2.71828" ], m: 2,
},
{
